...
interface TopBarProps {
  ...
  dataMode: 'mock' | 'token' | 'bigquery'
  onOpenUploader: () => void
}

export function TopBar({ trackCount, filteredCount, dataMode, onOpenUploader }: TopBarProps) {
  const { filters, setArtistFilter } = useDashboardStore()
  const activeArtist = filters.artistFilter

  const modeLabel = { mock: 'Mock data', token: 'Spotify live', bigquery: 'BigQuery' }[dataMode]
  const modeColor = { mock: 'text-amber-400', token: 'text-spotify-green', bigquery: 'text-blue-400' }[dataMode]
  ...
  return (
    <header className="h-12 bg-spotify-dark border-b border-spotify-border flex items-center px-4 gap-4 flex-shrink-0">
      ...
      {/* Track count — shows filtered/total when a filter is active */}
      ...
      <button
        ...
        className="text-xs px-3 py-1.5 bg-spotify-green text-black font-semibold rounded-full hover:bg-green-400 transition-colors flex-shrink-0"
      >
        Connect data
      </button>
    </header>
